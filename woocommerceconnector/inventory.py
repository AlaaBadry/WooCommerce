...
def get_inventory_levels(warehouses: Tuple[str], integration: str) -> List[_dict]:
    ...
    """
    # print(f'\n\n\n "sent wh" " " {warehouses,integration}\n\n\n')
    data = frappe.db.sql(
        f"""
            SELECT i.name as ecom_item, bin.item_code as item_code, integration_item_code, variant_id, actual_qty, warehouse, reserved_qty
            FROM `tabItem` i
                JOIN tabBin bin
                ON i.item_code = bin.item_code
            WHERE bin.warehouse in ({', '.join('%s' for _ in warehouses)})
                AND bin.modified < i.inventory_synced_on
                AND sync_qty_with_woocommerce = 1
        """,
        values=warehouses,
        as_dict=1,
    )
...
def update_inventory_sync_status(ecommerce_item, response, time=None):
    ...
    if time:
        frappe.db.set_value("Ecommerce Item", ecommerce_item,
                            "inventory_synced_on", time)
    if response:
        frappe.db.set_value(
            "Ecommerce Item", ecommerce_item, "response", response)
    # if status:
    #          frappe.db.set_value("Ecommerce Item", ecommerce_item, "status", status)
