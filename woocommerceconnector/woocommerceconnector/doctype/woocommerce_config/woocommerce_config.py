...
class WooCommerceConfig(Document):
    ...
    def validate_access_credentials(self):
        if not (
            ...
            and self.api_key
            and self.woocommerce_url
        ):
            frappe.msgprint(
                _(
                    "Missing value for Consumer Key, Consumer Secret, or woocommerce URL"
                ),
                raise_exception=woocommerceSetupError,
            )
...
def get_series():
    return {
        ...
        or "DN-woocommerce-",
        "item_code_naming_series": frappe.get_meta("Item").get_options("naming_series")
        or "Item-woocommerce-",
    }
