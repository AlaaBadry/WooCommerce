        ...
        if not item.woocommerce_product_id:
            ...
            if bin_since_last_sync > 0 or force != False:
                ...
                if item.woocommerce_product_id and item.variant_of:
                    ...
                    warehouse_map = {
                        wh.warehouse: wh.woolocation
                        for wh in woocommerce_settings.warehouses
                    }

                    inventory_levels = get_inventory_levels(
                        tuple(warehouse_map.keys()), item_code
                    )
                    for key, value in warehouse_map.items():
                        for d in inventory_levels:
                            if key == d["warehouse"]:
                                woolocation = value
                                meta = [{"key": woolocation, "value": d["actual_qty"]}]
                                ...
                                make_woocommerce_log(
                                    title="Update stock of {0}".format(item_code),
                                    status="Started",
                                    method="update_item_stock",
                                    message="Resource: {0}, data: {1}".format(
                                        resource, item_data
                                    ),
                                )
                                try:
