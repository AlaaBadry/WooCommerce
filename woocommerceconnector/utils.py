...
def get_inventory_levels(warehouses, item_code):
    """
    ...
    data = frappe.db.sql(
        f"""
            SELECT i.item_code as item_code, bin.item_code as item_code,  actual_qty, warehouse, reserved_qty
            FROM `tabItem` i
                JOIN tabBin bin
            ...
            WHERE bin.warehouse in ({', '.join('%s' for _ in warehouses)})
                ...
                AND i.item_code = %s
        """,
        values=warehouses + (item_code,),
        as_dict=1,
    )
    return data


def make_woocommerce_log(
    title="Sync Log",
    status="Queued",
    method="sync_woocommerce",
    message=None,
    exception=False,
    name=None,
    request_data={},
    ...
    if not name:
        ...
        if name:
            ...
            log = frappe.get_doc({"doctype": "woocommerce Log"}).insert(
